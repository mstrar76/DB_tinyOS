    ...
    try:
        ...
        for order in orders:
            try:
                ...
                orcar = order.get("orcar")
                orcado = order.get("orcado")
                ...
                conn.commit()
                print(f"Processed and saved Order ID: {order_id}")

            except Exception as e:
                conn.rollback()
                print(f"Error processing Order ID {order.get('id', 'N/A')}: {e}")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        json_data_file = 'detailed_10_orders.json'
