...
def fetch_and_process_orders(token, conn, cursor):
    """Extrai OS dos meses de 2013 e de julho a dezembro de 2016, utilizando lotes mensais."""
    headers = {"Authorization": f"Bearer {token}"}
    batch_size = 50

    # Define lotes mensais para 2013 (todos os meses) e 2016 (apenas julho a dezembro)
    batches = []

    # Lotes para 2013 (todos os meses)
    for month in range(1, 13):
        if month in [1, 3, 5, 7, 8, 10, 12]:
            batches.append((f"2013-{month:02d}-01", f"2013-{month:02d}-31"))
        elif month == 2:
            batches.append((f"2013-02-01", f"2013-02-28"))
        else:
            batches.append((f"2013-{month:02d}-01", f"2013-{month:02d}-30"))

    # Lotes para 2016 (julho a dezembro)
    for month in range(7, 13):
        if month in [7, 8, 10, 12]:
            batches.append((f"2016-{month:02d}-01", f"2016-{month:02d}-31"))
        elif month == 2:
            batches.append((f"2016-02-01", f"2016-02-29"))  # já processado anteriormente, mas mantido para referência
        else:
            batches.append((f"2016-{month:02d}-01", f"2016-{month:02d}-30"))

    # Agora batches contém todos os períodos desejados para extração
    ...
    for start_date, end_date in batches:
        ...
        for idx, order_id in enumerate(month_order_ids):
            ...
            try:
                ...
                # Registra a estrutura da resposta para debug
                logger.debug(f"API response structure for ID {order_id}: {list(response_json.keys())}")
                
                # Tenta diferentes possíveis estruturas da resposta
                if "ordemServico" in response_json:
                    order_data = response_json.get("ordemServico", {})
                elif "retorno" in response_json and "ordemServico" in response_json.get("retorno", {}):
                    order_data = response_json.get("retorno", {}).get("ordemServico", {})
                elif "dados" in response_json:
                    order_data = response_json.get("dados", {})
                else:
                    # Se não encontrar em nenhuma estrutura conhecida, tenta usar a resposta diretamente
                    # se ela tiver os campos esperados
                    if "id" in response_json and ("numeroOrdemServico" in response_json or "situacao" in response_json):
                        order_data = response_json
                    else:
                        order_data = {}
                
                if not order_data or "id" not in order_data:
                    logger.error(f"Empty or invalid order data received for ID {order_id}. Response keys: {list(response_json.keys())}")
